...
export const deleteProjectMutationFn = async ({
  workspaceId,
  projectId,
}: ProjectByIdPayloadType): Promise<AllProjectResponseType> => {
...
export const editTaskMutationFn = async ({
  ...
  workspaceId,
  data,
}: EditTaskPayloadType): Promise<AllTaskResponseType> => {
  const response = await API.put(
...
export const deleteTaskMutationFn = async ({taskId, workspaceId, projectId}: {taskId: string, workspaceId: string, projectId: string}   ): Promise<{message: string}> => {
  const response = await API.delete(`/task/${taskId}/project/${projectId}/workspace/${workspaceId}/delete`);
  return response.data;
};
